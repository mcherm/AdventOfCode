...
mod part_a {
    ...
    fn read_packet_file() -> Result<String, InputError> {
        let filename = "input/2021/input_16.txt";
...
mod part_b {
    ...
    impl OperatorType {
        fn from_type_id(id: u32) -> Self {
            match id {
                ...
                _ => panic!("Invalid operator type {}", id),
            }
        }
    }
    ...
    impl Packet {
        fn evaluate(&self) -> u64 {
            match self.fields {
                PacketFields::LiteralValue(val) => val,
                PacketFields::OperatorPacket(OperatorPacketFields{ref subpackets, ref operator_type, ..}) => {
                    let mut values = subpackets.iter().map(|x| x.evaluate());
                    match operator_type {
    ...
    impl fmt::Display for PacketFields {
        fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
            match self {
                ...
                PacketFields::OperatorPacket(OperatorPacketFields{length_type, operator_type, subpackets}) => {
                    write!(f, "Operator: {} {} [", length_type, operator_type)?;
                    let mut first_one = true;
                    for subpacket in subpackets {
                        if first_one {
                            first_one = false;
    ...
    impl fmt::Display for OperatorType {
        fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
            let symbol = match self {
                ...
                OperatorType::Minimum => "max",
                OperatorType::Maximum => "min",
    ...
    impl<'a> BinaryStream<'a> {
        ...
        fn read_packet(&mut self) -> Packet {
            ...
            let fields: PacketFields = match type_id {
                4 => self.read_data_packet_fields(),
                _ => self.read_operator_packet_fields(type_id),
            };
            Packet{version, type_id, fields}
        }
        ...
        fn read_operator_packet_fields(&mut self, type_id: u32) -> PacketFields {
    ...
    mod test {
        ...
        fn test_evaluate() {
            ...
            for (hex, expected_value) in test_cases {
                let packet: Packet = BinaryStream::new(hex).read_packet();
